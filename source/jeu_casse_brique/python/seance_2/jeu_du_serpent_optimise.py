...
class Jeu:
    ...
    def update(self):
        ...
        if pyxel.frame_count % self.FRAME_REFRESH == 0 and not self.game_over:
            ...
            if pyxel.btn(pyxel.KEY_RIGHT) and self.snake.direction in ([0,1],[0,-1]):
                self.snake.direction = [1,0]
            elif pyxel.btn(pyxel.KEY_LEFT) and self.snake.direction in ([0,1],[0,-1]):
                self.snake.direction = [-1,0]
            elif pyxel.btn(pyxel.KEY_UP) and self.snake.direction in ([1,0],[-1,0]):
                self.snake.direction = [0,-1]
            elif pyxel.btn(pyxel.KEY_DOWN) and self.snake.direction in ([1,0],[-1,0]):
                self.snake.direction = [0,1]
            
            # test si le serpent sort du cadre de jeu ou se mord
            tete = self.snake.serpent[0]
            if (tete in self.snake.serpent[1:]) \
