...
class Jeu:
    def __init__(self):
        ...
        self.aliments = [Aliment() for i in range(20)]
        for i in range(len(self.aliments)):
            if randint(0,2) == 0:
                self.aliments[i].empoisonner()
        ...
        for aliment in self.aliments:
            ...
            if self.gober_aliment(aliment):
                if aliment.nocif:
                    ...
                    self.glouton.maigrir()
                    self.glouton.vitesse *= 0.9
                    self.aliments.remove(aliment)
                    self.score += -2
                else:
                    self.glouton.grossir()
                    self.glouton.vitesse *= 1.05
                    self.aliments.remove(aliment)
                    self.score += 1
    ...
    def afficher_aliments(self):
        ...
        for aliment in self.aliments:
            pyxel.circ(aliment.centre_x,aliment.centre_y,aliment.rayon,aliment.couleur)
    ...
    def draw(self):
        ...
        if self.aliments != []:
            ...
            pyxel.text(40,40,"FIN DE PARTIE",7)
            pyxel.text(45,50,"SCORE: "+ str(self.score),7)
            pyxel.text(45,60,"GROSSEUR: "+ str(self.glouton.long_x),7)
            pyxel.text(45,70,"VITESSE: "+ str(round(self.glouton.vitesse,2)),7)
            pyxel.text(40,90,"SCORE FINAL: "+ str(int(self.score*self.glouton.long_x+100*round(self.glouton.vitesse,2))),7)
